...
✨ ImageTransfer solves the frustration of uploading high-resolution photos to sites with strict size caps. Modern cameras create multi-megabyte images that exceed common upload limits, and off-the-shelf compressors rarely hit a specific target size. ImageTransfer lets you set the exact output size so your photos meet website requirements without guesswork.
...
- Load `ImageTransfer.sln` solution and set `Wpf` project as startup.

## Usage

1. Download the latest version of ImageTransfer from the [Releases](https://github.com/Burnside999/ImageTransfer/releases) page.
2. ImageTransfer requires the .NET 8.0 runtime. Run `setup-doenet.bat` to install it.
3. Launch `ImageTransfer.exe` to start the application.
